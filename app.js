...
// ⚡ VERIFICAÇÃO AUTOMÁTICA DA TABELA (CORRIGIDA)
...
inicializarBanco().then(() => {
    // Teste de conexão após inicialização
    pool.query('SELECT NOW()')
        .then(() => console.log('✅ PostgreSQL conectado e tabela verificada'))
        .catch(err => {
...
app.get('/admin/pedidos', auth, async(req, res) => {
    ...
    try {
        ...
        res.status(500).json({ error: 'Erro ao carregar pedidos. Tabela pode não existir.' });
    } finally {
        if (client) client.release();
    }
});
...
app.get('/admin/stats', auth, async(req, res) => {
    ...
    try {
        ...
        res.json({ totalPedidos: 0, receitaTotal: 0, dataConsulta: new Date().toLocaleString('pt-BR') });
    } finally {
        if (client) client.release();
    }
});
...
// Rota para verificar se tabela existe
